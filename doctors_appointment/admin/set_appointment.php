...
<div class="container-fluid">
	<div class="col-lg-12">
		...
		<?php endif; ?>
			<div class="form-group">
				<label for="" class="control-label">Pacient</label>
			...
			<div class="form-group">
				<label for="" class="control-label">Ora</label>
			...
			<div class="form-group">
				...
				<select class="browser-default custom-select" name="status">
					<option value="0" <?php echo isset($status) && $status == 0 ? "selected" : '' ; ?>>Cerere</option>
					<option value="1" <?php echo isset($status) && $status == 1 ? "selected" : '' ; ?>>Confirmat</option>
					<option value="2" <?php echo isset($status) && $status == 2 ? "selected" : '' ; ?>>Reprogramat</option>
					<option value="3" <?php echo isset($status) && $status == 3 ? "selected" : '' ; ?>>Încheiat</option>
				</select>
			</div>
			...
			<div class="col-md-12 text-center">
				<button class="btn-primary btn btn-sm col-md-4">Modifică</button>
				<button class="btn btn-secondary btn-sm col-md-4  " type="button" data-dismiss="modal" id="">Anulare</button>
			</div>
		</form>
	</div>
</div>
	...
	$("#manage-appointment").submit(function(e){
		...
		$.ajax({
			...
			success:function(resp){
				...
				if(resp.status == 1){
					alert_toast("Cerere deschisă cu succes");
					// end_load();
					$('.modal').modal("hide");
					setTimeout(function(){
						location.reload();
